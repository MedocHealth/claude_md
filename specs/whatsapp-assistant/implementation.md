...
## Status: M0 in progress (session 3, 2026-09-05) — identity chain, webhook and mirrors built, unit-tested and pushed on branches `feat/assistant-audience` (no PRs yet); nothing runtime-verified against Meta; Day-0 checklist items 2–9 still owner-blocked
...
2. [~] Create GitHub repo `medoc-assistant-service` — created private 2026-09-05 (https://github.com/MedocHealth/medoc-assistant-service), `main` pushed; engineer write rights still to grant; all three engineers have write rights on their owned repos (§1 ownership table); the manager has merge rights everywhere.
...
- **Pushed 2026-09-05** on owner instruction: all six `feat/assistant-audience` branches, `claude_md` `feature/team-context-system`, and the new private repo `MedocHealth/medoc-assistant-service` (`main`). No PRs opened yet.
- **Not done / blocked:** Atlas `$vectorSearch` spike and Vertex `asia-south1` smoke (no credentials); Meta app + test number; Vault entries (`SERVICE_API_KEYS` on auth-service, `secret/medoc-assistant-service`); the HTTP round trip of the M0 demo (needs all three services up with real env). The probe did run against the local `.local-msf-stack` dump (1 hospital `hos_AXON01`, 1 role, 1 access row): `rolesMissingAuthMe: 0`, `employeesNotCovered: 0` — the script works end to end, but one seeded hospital is not the coverage evidence the spec asks for; the dev-Atlas run is still owed.
...
- **Fable review (read-only agent, all three diffs): no blockers.** Fixed in follow-up commits: mint cross-checks credential vs link (`bd8199a`); HPlus validates `x-assistant-message-id` as a wamid and logs error name/code only — jose attaches decoded claims to its errors (`35facd51`); service: ledger compensation on publish failure, redelivery guard, whitelist fail-closed outside prod, DLQ TTL, profile name dropped, type guards at HTTP/Redis boundaries. **Left open (documented):** revoke latency = token expiry until `TokenService.forget` is wired to the M1 revoke hook; `upsertLink` trusts the caller's proof of phone possession — the M1 OTP step must land before `/api/link/verify` is reachable; `ASSISTANT_TOKEN_EXPIRY` is parsed at first mint, not boot; Redis token key embeds the phone (hash it in M1).
